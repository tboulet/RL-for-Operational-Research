...
class SARSA(BaseRLAlgorithm):
    ...
    def update(
        ...
        if self.has_done_at_least_one_episode_step:
            ...
            assert self.last_transition is not None, "The last transition should not be None"
            last_state, last_action, last_reward, last_next_state, last_done = self.last_transition
            assert state == last_next_state, "The current state should be the next state of the last transition"
            assert not last_done, "The last transition should not be done, otherwise the episode is done and we wouldn't be here"
        ...
        return {
            **{f"Q(s={s}, a={a})": self.q_values[s][a] for s in self.q_values for a in self.q_values[s]},
            **{
                f"1(A={a} in S={state})": int(a == action)
                for a in self.q_values[state].keys()
            },
            "epsilon": self.policy.epsilon.get_value(),
        }
