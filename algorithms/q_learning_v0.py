...
class Q_learning_v0(BaseRLAlgorithm):
    ...
    def act(
        ...
        if str(state) not in self.Q:
            if self.value is None:
                self.Q[str(state)] = {
                    action: np.random.uniform(self.min_Q, self.max_Q)
                    for action in available_actions
                }
            else:
                self.Q[str(state)] = {
                    action: self.value for action in available_actions
                }
    ...
    def update(
        ...
        if done:
            self.Q[str(state)][action] = (1 - self.alpha) * self.Q[str(state)][
                action
            ] + self.alpha * reward
            self.value = None
        else:
